...
impl RomFile {
    pub fn load(path: &str) -> Result<RomFile, IOError> {
        let mut cache = ROM_CACHE
            .lock()
            .expect("Rom cache couldn't be unlocked, this should not happen");
        ...
        println!(
            "Loaded ROM with MBC type {:02x} ROM SIZE (ID) {:02x} RAM SIZE (ID) {:02x}",
            bytes[0x147], bytes[0x148], bytes[0x149]
        );
...
impl<'de> Visitor<'de> for RomFileVisitor {
    ...
    {
        RomFile::load(path)
            .map_err(|_| E::custom(format!("ROM could not be loaded from path: {}", path)))
    }
}
