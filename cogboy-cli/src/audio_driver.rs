...
pub fn init(snd_rx: Receiver<SoundMessage>,
    ...
    message_tx.send(ControlMessage::Tick((FRAMES_PER_BUFFER as f64 * (1 << 22) as f64 /
                                    SAMPLE_RATE) as i32)).unwrap();
    ...
    let callback = move |pa::OutputStreamCallbackArgs { buffer, frames, .. }| {
        ...
        if state.paused {
            ...
            if queue_len < queue_target {
                state.message_tx.send(ControlMessage::Tick(((queue_target - queue_len) as f64 / state.subsample_step) as i32)).unwrap();
            }
        }
